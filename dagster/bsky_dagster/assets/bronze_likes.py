"""The Kafka -> Iceberg bronze likes archiver asset.

Mirror of ``bronze.py`` for the like event type. Same streaming-safety contract: batched
...
like-specific decode/batch/schema/topic differ.
"""
...
from bsky_dagster.transforms.bronze_schema_likes import (
    likes_iceberg_schema,
    likes_partition_spec,
)
from bsky_dagster.transforms.decode import DlqRow
from bsky_dagster.transforms.decode_likes import decode_like
from dagster import DailyPartitionsDefinition, MaterializeResult, asset

log = structlog.get_logger("bsky_dagster.bronze_likes")

likes_daily_partitions = DailyPartitionsDefinition(start_date="2026-06-01", end_offset=1)


@asset(
    name="likes_bronze",
    partitions_def=likes_daily_partitions,
    description="Raw append-only Iceberg archive of bsky.likes.v1 (Kafka -> Iceberg bronze).",
    group_name="bronze",
)
def likes_bronze() -> MaterializeResult:
    settings = Settings()
    catalog = load_catalog(settings)
    table = ensure_table(
        catalog,
        settings,
        table_name=settings.bronze_table_likes,
        schema=likes_iceberg_schema(),
        spec=likes_partition_spec(),
    )
    deserializer = build_deserializer(settings, "bsky.likes.v1.avsc")
    consumer = build_consumer(
        settings, topic=settings.topic_likes, group=settings.bronze_consumer_group_likes
    )
    dlq = DlqProducer(settings)

    batch = LikesBatch(settings.bronze_batch_max_rows, settings.bronze_batch_max_seconds)
    archived = 0
    dlq_count = 0
    ...
    def flush() -> None:
        ...
        _commit(consumer, settings.topic_likes, batch.offsets())
        archived += len(batch)
        batch.clear()
        last_flush = time.monotonic()
    ...
    try:
        while archived + len(batch) < settings.bronze_max_rows_per_run:
            ...
            if isinstance(row, DlqRow):
                dlq.send(row, intended_topic=settings.topic_likes)
                dlq_count += 1
                _commit(consumer, settings.topic_likes, {partition: offset})
                continue
    ...
    log.info("likes_bronze_done", archived=archived, dlq=dlq_count, table_total=total)
