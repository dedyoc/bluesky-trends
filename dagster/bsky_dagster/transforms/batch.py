"""Pure batch accumulator for the Kafka -> Iceberg bronze write.

Holds decoded BronzeRows until a flush threshold is hit (>=N rows OR elapsed >=T seconds),
satisfying the "batched, never row-by-row" rule. Converts the held rows to a single
pyarrow Table (one Iceberg append per flush -> one data file per partition, avoiding the
Iceberg analogue of ClickHouse's "too many parts").

It also tracks the max offset seen per partition so the caller can commit offsets ONLY
after the append succeeds (write-before-commit). No I/O here — the accumulator never
touches Kafka or Iceberg; it only decides and shapes.
"""
...
from bsky_dagster.transforms.bronze_schema import bronze_arrow_schema
...
class BronzeBatch:
    ...
    def to_arrow(self) -> pa.Table:
        """Materialize the held rows as a pyarrow Table matching the bronze schema."""
        ...
        for r in self._rows:
            ...
            columns["langs"].append(r.langs)
            columns["reply_parent"].append(r.reply_parent)
            columns["reply_root"].append(r.reply_root)
